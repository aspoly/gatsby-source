...
class ContentMesh {
        ...
        return relations.reduce((bag, relation) => {
            ...
            if (!destTable || !srcTable) {
                ...
                if (!collection_many.match(/^directus/) || !collection_one.match(/^directus/)) {
                    utils_1.log.warn('Unable to resolve Directus relation', {
                        config: relation,
                        srcTable,
                        destTable,
                    });
                    utils_1.log.warn('Have these collections in memory', {
    ...
    _buildM2MRelations(relations = []) {
        ...
        return Object.values(junctions).reduce((bag, [a, b]) => {
            ...
            if (!destTable || !srcTable || !junctionTable) {
                utils_1.log.warn('Unable to resolve Directus junction. Missing collections', {
                    destTable,
                    srcTable,
                    junctionTable,
                    a,
                ...
                utils_1.log.warn('Have these tables in memory', {
                    collections: Object.keys(this._collections),
                });
    ...
    _shouldProcessRelation(
    // eslint-disable-next-line @typescript-eslint/camelcase
    { collection_many, collection_one, junction_field }, type) {
        // eslint-disable-next-line @typescript-eslint/camelcase
        if (collection_many.match(/^directus/) && collection_one.match(/^directus/)) {
            return false;
        }
